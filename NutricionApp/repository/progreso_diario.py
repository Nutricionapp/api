...
def create(db: Session):
    ...
    for rutina in rutinas:
        ...
        for i in range(0, len(comidas)):
            ...
            recetas_calorias = db.query(models.Receta).filter(models.Receta.cantidad_calorias < calorias + 350,
                                                              models.Receta.cantidad_calorias > calorias - 350,
                                                              models.Receta.tipo_comida == comidas[i]).all()
            id_recetas = [x.id_receta for x in recetas_calorias]
            for receta in id_recetas:
                ingredientes = db.query(models.IngredienteReceta).filter(
                    models.IngredienteReceta.id_receta == receta).all()
...
def update(id_usuario: int, db: Session):
    rutina_usuario = db.query(models.Rutina).filter(models.Rutina.id_usuario == id_usuario,
